...
Where:
  ...
  + `body of the function` is the code that operates with the function arguments to yield a result. This can be one line of code, or a thousand, it all comes down to the function's objective. In any case, the `body of the function` must return an object (or an error if something went wrong) that will be the function output. I have called this output `function_output`, but it can have any name you want.
  + `return(function_output)` tells the function that the value to return is stored in the variable `function_output` (this variable can have any name you need). This `function_output` must be the result of the operations performed by the `body of the function`. It is important to remember that R functions can return one object only.
...
So far so good! From here we build the functions `sorensen_beta()` and `simpson_beta()` making sure they can accept the output of `abc()`, and return it with an added slot. No matter what you might have heard about `return()`, it must be included in these functions (you can try them without `return()` to learn why).
